...
*
*******************************************************************************/
cy_en_syspm_status_t Cy_SCB_UART_HibernateCallback(cy_stc_syspm_callback_params_t const *callbackParams, cy_en_syspm_callback_mode_t mode)
{
...
cy_en_scb_uart_status_t Cy_SCB_UART_Receive(CySCB_Type *base, void *buffer, uint32_t size, cy_stc_scb_uart_context_t *context)
{
    ...
    if (0UL == (context->rxStatus & CY_SCB_UART_RECEIVE_ACTIVE))
    {
        ...
        if (locSize > 0UL)
        {
            ...
            Cy_SCB_SetRxFifoLevel(base, (locSize > halfFifoSize) ? (halfFifoSize - 1UL) : (locSize - 1UL));
