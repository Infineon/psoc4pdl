...
* the ILO. Need to consider ILO accuracy while configuring WDT intervals to 
...
* After the ILO is enabled, ensure that the WDT is disabled by
* calling the Cy_WDT_Disable() function. Set the WDT match
...
__STATIC_INLINE void Cy_WDT_MaskInterrupt(void)
{
    SRSSLT_SRSS_INTR_MASK &= ~SRSSLT_SRSS_INTR_MASK_WDT_MATCH_Msk;
}
